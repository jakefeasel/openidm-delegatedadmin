...
(function (resource, queryId, page, limit, orderBy, orderByDir, searching) {
    ...
    if (context.security.authorizationId.roles.indexOf('openidm-admin') === -1 &&
        context.security.authorizationId.roles.indexOf('openidm-org-admin') !== -1) {

        userData = openidm.read(context.security.authorizationId.component + '/' + context.security.authorizationId.id);
        pagingQueryArgs.org = userData.org;

        pagingQueryArgs._queryId = pagingQueryId + "-for-org";
    } else {
